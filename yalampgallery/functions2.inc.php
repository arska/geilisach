<?
function makethumb ($id, $s, $h=0, $v=0, $debug=0) { // aufzurufen mit makethumb(ID, 640) oder makethumb(ID, 0, 640, 480)
	...
	if ($id) {
		...
		{
			if (($orig = imagecreatefromjpeg($bild_info['path'])) == false) {
		...
		{
			if (($orig = imagecreatefromgif($bild_info['path'])) == false) {
		...
		{
			if (($orig = imagecreatefrompng($bild_info['path'])) == false) {
		...
		if (!imagejpeg($thumb,$thumbpath)) { // mime-type ist im sql hard-coded
...
function getimage ($id, $linkto=0, $s=0, $addpath="", $linktarget="", $debug = 0) { // $linkto=0 -> no link; $s=0 -> originalgrösse; $addpath: path zu output.php; $linktarget: target="" für $linkto
	...
	if (mysql_num_rows($result) < 1) { // keine solche grösse vorhanden
		...
		if (mysql_num_rows($result) < 1) { // das bild mit $id gibt's gar nicht !
			...
			$thumbid = makethumb($id, $s, 0, 0, $debug); // macht ein bild mit den richtigen grössen
